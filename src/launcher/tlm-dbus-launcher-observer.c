        ...
        GError **error)
{
    ...
    DBG ("start process with path %s and args %s", app_abs_path, args);
    ...
    execvp (app_abs_path, split_args);
...
tlm_dbus_launcher_stop_app (
        ...
        GError **error)
{
    DBG ("stop app with id %d", app_id);
    g_return_if_fail (self && TLM_IS_DBUS_LAUNCHER_OBSERVER(self));
    struct ProcessObject *obj = g_hash_table_lookup (
    		self->priv->launched_processes, GUINT_TO_POINTER (app_id));

    if (obj) {
    	_stop_process (app_id, obj, self);
    } else {
