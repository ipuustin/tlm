        ...
        TlmDbusLauncherAdapterClass *klass)
{
    ...
    signals[SIG_APP_LAUNCHED] = g_signal_new ("app-launched",
    ...
    signals[SIG_APP_TERMINATED] = g_signal_new ("app-terminated",
            TLM_TYPE_LAUNCHER_ADAPTER,
            G_SIGNAL_RUN_LAST,
            0,
            NULL,
        ...
        gpointer emitter)
{
    ...
    DBG ("launch - application path %s args %s", app_abs_path, args);
    rval = tlm_dbus_launcher_launch_app (self->priv->observer, app_abs_path,
    		args, &appid, &error);
    if (rval) {
    	tlm_dbus_launcher_complete_launch_app (self->priv->dbus_obj,
    	                invocation, appid);
    } else {
    	g_dbus_method_invocation_return_gerror (invocation, error);
    	g_error_free (error);
        ...
        gpointer emitter)
{
    ...
    DBG ("stopping - app %u", app_id);

    rval = tlm_dbus_launcher_stop_app (self->priv->observer, app_id, &error);
    if (rval) {
    	tlm_dbus_launcher_complete_stop_app (self->priv->dbus_obj,
    	                invocation);
    } else {
    	g_dbus_method_invocation_return_gerror (invocation, error);
    	g_error_free (error);
        ...
        GDBusConnection *bus_connection)
{
    ...
    g_signal_connect_swapped (adapter->priv->dbus_obj,
        "handle-launch-app", G_CALLBACK (_handle_launch_app), adapter);
    g_signal_connect_swapped (adapter->priv->dbus_obj,
        "handle-stop-app", G_CALLBACK(_handle_stop_app), adapter);
    g_signal_connect_swapped (adapter->priv->dbus_obj,
        "handle-list-apps", G_CALLBACK(_handle_list_apps), adapter);
